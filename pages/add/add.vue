<template>
	<view class="app-container">
      ...
      <view class="container_row">
        ...
        <text class="row_value">{{'童装、麻花...'}}<text class="row_icon">{{'>'}}</text></text>
			</view>
      <view class="container_row">
			  <text class="row_title">录入时间</text>
        <text class="row_value">{{'2020-12-01'}}<text class="row_icon">{{'>'}}</text></text>
			</view>
      <view class="container_row">
			  <text class="row_title">衣服总数</text>
        <text class="row_value">{{'120'+'/件'}}<text class="row_icon">{{'>'}}</text></text>
			</view>
      <view class="container_col">
			  <view class="col_title">申请备注</view>
        <view class="col_textarea">
          <textarea placeholder="在此填写备注"  placeholder-class="placeholder" maxlength="200" @input = "watchInput" v-model="name" />
          <view class="textarea_num"><text style="color: #FB231F;">{{remnant}}</text>/200</view>
        </view>
			</view>
      <view class="container_col">
